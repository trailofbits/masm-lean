...
#eval do
  ...
  | some s' =>
    ...
    unless s'.stack == [1, 0, 3, 0] do
      panic! "divmod 10/3: unexpected output stack"
...
#eval do
  ...
  | some s' =>
    unless s'.stack == [2, 0, 14, 0] do
      panic! "divmod 100/7: unexpected output stack"
  | none => panic! "divmod 100/7 should not fail"

-- ============================================================================
-- Tier 6: Cross-validation with miden-vm test vectors
-- ============================================================================
-- These tests reproduce EXACT test vectors from the miden-vm Rust
-- test suite (crates/lib/core/tests/math/u64_mod.rs) and run the
-- same MASM procedures through our Lean semantics model. Any
-- mismatch means our model diverges from the real VM.
--
-- Ref file: miden-vm/crates/lib/core/tests/math/u64_mod.rs

-- Complete u64 ProcEnv for testing (extends the proof ProcEnv)
...
private def runU64Adv (proc : List Op) (stk : List Felt)
    (adv : List Felt) (fuel := 100) : Option MidenState :=
  execWithEnv testU64ProcEnv fuel (mkStateAdv stk adv) proc

-- Cross-val: wrapping_add (u64_mod.rs line 32-52)
-- a = 0x00000002_00000005 (hi=2,lo=5), b = 0x00000001_00000003
-- c = a+b = 0x00000003_00000008 (hi=3,lo=8)
-- Input: [a_lo, a_hi, b_lo, b_hi] = [5, 2, 3, 1]
-- Output: [c_lo, c_hi] = [8, 3]
#eval do
...
-- Cross-val: lt (u64_mod.rs line 270-288)
-- [b_lo, b_hi, a_lo, a_hi] computes a < b
-- a=0,b=0 -> 0; a=0,b=1 -> 1; a=1,b=0 -> 0
#eval do
...
-- Cross-val: lte (u64_mod.rs line 290-316)
#eval do
  let check (stk : List Felt) (expected : Felt) (msg : String) : IO Unit := do
...
-- Cross-val: gt (u64_mod.rs line 318-336)
#eval do
  let check (stk : List Felt) (expected : Felt) (msg : String) : IO Unit := do
...
-- Cross-val: gte (u64_mod.rs line 338-364)
#eval do
  let check (stk : List Felt) (expected : Felt) (msg : String) : IO Unit := do
...
-- Cross-val: min (u64_mod.rs line 366-384)
...
-- Cross-val: max (u64_mod.rs line 386-404)
#eval do
  let check (stk : List Felt) (exp : List Felt) (msg : String) : IO Unit := do
...
-- Cross-val: eq (u64_mod.rs line 406-432)
...
-- Cross-val: neq (u64_mod.rs line 434-460)
#eval do
  let check (stk : List Felt) (expected : Felt) (msg : String) : IO Unit := do
...
-- Cross-val: eqz (u64_mod.rs line 462-483)
-- [a_lo, a_hi] -> [flag]
#eval do
  let check (stk : List Felt) (expected : Felt) (msg : String) : IO Unit := do
...
-- Cross-val: advice_push_u64div_two_pushes (u64_mod.rs line 526-549)
-- a=123, b=10 => q=12, r=3
-- Advice: [q_hi=0, q_lo=12, r_hi=0, r_lo=3]
-- After two advPush.2: stack=[r_lo=3, r_hi=0, q_lo=12, q_hi=0, b_lo=10, ...]
open Miden.Core.U64 in
open MidenLean.Proofs in
#eval do
  let stk := [(10:Felt), 0, 123, 0]  -- [b_lo, b_hi, a_lo, a_hi]
  let adv := [(0:Felt), 12, 0, 3]    -- [q_hi, q_lo, r_hi, r_lo]
  let s := mkStateAdv stk adv
  let r := execWithEnv u64ProcEnv 50 s divmod
  match r with
  | some s' =>
...
-- Cross-val: clz (u64_mod.rs line 1207-1231)
-- a=0 -> 64
#eval do
  match runU64 Miden.Core.U64.clz [0, 0] with
  | some s =>
    unless s.stack[0]! == (64 : Felt) do
      panic! "CROSS-VAL clz: clz(0)=64"
  | none => panic! "clz should not fail"

-- Cross-val: ctz (u64_mod.rs line 1258-1281)
-- a=0 -> 64
#eval do
  match runU64 Miden.Core.U64.ctz [0, 0] with
  | some s =>
    unless s.stack[0]! == (64 : Felt) do
      panic! "CROSS-VAL ctz: ctz(0)=64"
  | none => panic! "ctz should not fail"

-- Cross-val: clo (u64_mod.rs line 1284-1307)
...
#eval do
  ...
  | some s =>
    unless s.stack[0]! == (64 : Felt) do
      panic! "CROSS-VAL clo: clo(0xFFFFFFFFFFFFFFFF)=64"
  | none => panic! "clo should not fail"

-- Cross-val: cto (u64_mod.rs line 1310-1333)
-- a=0xffffffffffffffff -> 64
#eval do
  let maxu32 : Felt := Felt.ofNat (2^32 - 1)
  ...
  | some s =>
    unless s.stack[0]! == (64 : Felt) do
      panic! "CROSS-VAL cto: cto(0xFFFFFFFFFFFFFFFF)=64"
  | none => panic! "cto should not fail"

-- Cross-val: shl (u64_mod.rs line 934-982)
-- a=1 (lo=1,hi=0), shift=32 -> (lo=0,hi=1)
#eval do
  -- [shift, a_lo, a_hi, trailing...]
  match runU64 Miden.Core.U64.shl [32, 1, 0, 5] with
  | some s =>
    unless s.stack.take 3 == [0, 1, 5] do
      panic! "CROSS-VAL shl: 1<<32 should give [0,1,5]"
  | none => panic! "shl should not fail"

-- Cross-val: shr (u64_mod.rs line 985-1041)
-- a=0x00000001_00000001 (hi=1,lo=1), shift=1
-- result = 0x00000000_80000000 (hi=0,lo=2^31)
#eval do
