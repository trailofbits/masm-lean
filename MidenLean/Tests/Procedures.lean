...
private def checkStackPrefix (result : Option Concrete.State) (expected : List Felt) : Bool :=
  match result with
...
#eval do
  ...
  let r := execProcedure emptyEnv 20 s Miden.Core.U64.lt
...
#eval do
  let s := mkState [5, 0, 7, 0]
  let r := execProcedure emptyEnv 20 s Miden.Core.U64.gt
...
#eval do
  ...
  let r := execProcedure u64ProcEnv 30 s Miden.Core.U64.lte
...
#eval do
  ...
  let r := execProcedure u64ProcEnv 30 s Miden.Core.U64.gte
...
#eval do
  ...
  let r := execProcedure u64ProcEnv 30 s Miden.Core.U64.overflowing_sub
...
#eval do
  ...
  let r := execProcedure emptyEnv 20 s Miden.Core.U64.overflowing_add
...
#eval do
  ...
  let r := execProcedure emptyEnv 50 s Miden.Core.U64.shr
...
#eval do
  let s := mkState [4, 1, 0]
  let r := execProcedure u64ProcEnv 50 s Miden.Core.U64.shl
