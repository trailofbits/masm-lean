...
macro_rules
  | `(tactic| miden_step) =>
    `(tactic| first
      ...
      | (rw [stepU32WrappingAdd (ha := by assumption) (hb := by assumption)]; miden_bind)
      | (rw [stepU32WrappingAdd3 (ha := by assumption) (hb := by assumption) (hc := by assumption)]; miden_bind)
      ...
      | (rw [stepU32RotrImm (ha := by assumption) (hn := by decide)]; miden_bind)
      | (rw [stepU32ShrImm  (ha := by assumption) (hn := by decide)]; miden_bind)
