...
macro_rules
  | `(tactic| miden_step) =>
    `(tactic| first
      ...
      -- U32 shifts/rotations immediate (with isU32 + bound hypotheses)
      | (rw [stepU32ShrImm (ha := by assumption) (hn := by omega)]; miden_bind)
      | (rw [stepU32RotrImm (ha := by assumption) (hn := by omega)]; miden_bind)
      | (rw [stepU32RotlImm (ha := by assumption) (hn := by omega)]; miden_bind)
      ...
      | (rw [stepU32Popcnt (ha := by assumption)]; miden_bind)
