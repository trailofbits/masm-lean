...
/-- `u64::neq` tests inequality of two u64 values, limb by limb.
    ...
    summary for reflective callers and as the basis for `u64_neq_correct`. -/
@[miden_exec_summary]
theorem u64_neq_exec (env : ProcEnv) (fuel : Nat)
    (b_lo b_hi a_lo a_hi : Felt) (rest : List Felt) (s : Concrete.State)
    (hs : s.stack = b_lo :: b_hi :: a_lo :: a_hi :: rest) :
    execProcedure env (fuel + 1) s Miden.Core.U64.neq =
    some (s.withStack (
...
theorem u64_neq_correct (a b : U64) (rest : List Felt) (s : Concrete.State)
    (hs : s.stack = b.lo.val :: b.hi.val :: a.lo.val :: a.hi.val :: rest) :
    execProcedure emptyEnv 10 s Miden.Core.U64.neq =
    some (s.withStack (
      (if a != b then (1 : Felt) else 0) :: rest)) := by
  have h := u64_neq_exec emptyEnv 9 b.lo.val b.hi.val a.lo.val a.hi.val rest s hs
