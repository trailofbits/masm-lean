...
/-- `u64::gte` compares two u64 values.
    Input stack:  [b_lo, b_hi, a_lo, a_hi] ++ rest
    Output stack: [result] ++ rest
    where result = 1 iff a ≥ b (as u64), else 0.
    Computed as !(a < b).
    Parametric in `fuel` so this lemma serves both as a callee summary for
    reflective callers and as the basis for `u64_gte_correct`. The env is
    fixed to `u64ProcEnv` because the proof resolves the `exec lt` call by
    unfolding that environment. -/
@[miden_exec_summary]
theorem u64_gte_exec
    (fuel : Nat)
    (a_lo a_hi b_lo b_hi : Felt) (rest : List Felt) (s : Concrete.State)
    (hs : s.stack = b_lo :: b_hi :: a_lo :: a_hi :: rest)
    (ha_lo : a_lo.isU32 = true) (ha_hi : a_hi.isU32 = true)
    (hb_lo : b_lo.isU32 = true) (hb_hi : b_hi.isU32 = true) :
    execProcedure u64ProcEnv (fuel + 2) s Miden.Core.U64.gte =
    some (s.withStack (
...
theorem u64_gte_correct (a b : U64) (rest : List Felt) (s : Concrete.State)
  ...
  rw [u64_gte_exec 18 a.lo.val a.hi.val b.lo.val b.hi.val rest s hs a.lo.isU32 a.hi.isU32 b.lo.isU32 b.hi.isU32]
