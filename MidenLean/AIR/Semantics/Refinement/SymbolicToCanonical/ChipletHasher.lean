...
theorem bridge_chiplet_hasher_0 (r : AirRow) :
  ...
  -- Structural mismatch: the extracted symbolic hasher state starts at `col 55`,
  -- while the canonical subsystem uses `stateCol lane0 = col 56`.
  sorry
...
theorem bridge_chiplet_hasher_1 (r : AirRow) :
  ...
  -- Structural mismatch: the extracted symbolic hasher state uses cols `55..66`,
  -- while the canonical subsystem uses `56..67`.
  sorry
...
theorem bridge_chiplet_hasher_12 (r : AirRow) :
  ...
  -- Structural mismatch: the external-round state lanes are shifted by one
  -- column between the extracted symbolic and canonical layouts.
  sorry
...
theorem bridge_chiplet_hasher_24 (r : AirRow) :
  ...
  -- Structural mismatch: the internal-round state lanes are shifted by one
  -- column between the extracted symbolic and canonical layouts.
  sorry
...
theorem bridge_chiplet_hasher_39 (r : AirRow) :
  ...
  -- No state-column mismatch appears here, but the remaining gate algebra
  -- still needs a dedicated normalization lemma for `fOut + fOutNext`.
  sorry
...
theorem bridge_chiplet_hasher_41 (r : AirRow) :
  ...
  -- No state-column mismatch appears here, but the row-31 continuation gate
  -- still needs a dedicated normalization lemma for `fContinuation`.
  sorry
...
theorem bridge_chiplet_hasher_42 (r : AirRow) :
  ...
  -- No state-column mismatch appears here, but the selector-only row-31 gate
  -- still needs a small normalization lemma for the current output flag.
  sorry
...
theorem bridge_chiplet_hasher_43 (r : AirRow) :
  ...
  -- Structural mismatch: symbolic ABP capacity lanes are indexed as `63..66`,
  -- but canonical capacity lanes are `64..67`.
  sorry
...
theorem bridge_chiplet_hasher_47 (r : AirRow) :
  ...
  -- Structural mismatch: the extracted symbolic node index is `col 67`,
  -- while the canonical subsystem uses `nodeIndexCol = col 68`.
  sorry
...
theorem bridge_chiplet_hasher_48 (r : AirRow) :
  ...
  -- Structural mismatch: the extracted symbolic node index uses `col 67`,
  -- while the canonical direction bit is defined from `col 68`.
  sorry
...
theorem bridge_chiplet_hasher_49 (r : AirRow) :
  ...
  -- Structural mismatch: the extracted symbolic node index uses `col 67`,
  -- while the canonical node-index column is `col 68`.
  sorry
...
theorem bridge_chiplet_hasher_50 (r : AirRow) :
  ...
  -- Structural mismatch: the Merkle absorb constraints depend on both the
  -- shifted symbolic node-index column and the shifted state layout.
  sorry
...
theorem bridge_chiplet_hasher_54 (r : AirRow) :
  ...
  -- Structural mismatch: symbolic digest and rate placements are shifted by
  -- one column, and the direction bit is taken from the shifted node index.
  sorry
