...
theorem execInstruction_sound_padw
  ...
  simp only [execInstruction] at hexec
  have heq := Option.some.inj hexec
  have hss : ss' = { stack := .lit 0 :: .lit 0 :: .lit 0 :: .lit 0 :: ss.stack } :=
    (congrArg Prod.fst heq).symm
  have hpc : preconds = [] := (congrArg Prod.snd heq).symm
  subst hss; subst hpc
  exact ⟨cs.withStack (0 :: 0 :: 0 :: 0 :: cs.stack),
    by simp only [MidenLean.execInstruction, execPadw, hmodels, MidenState.withStack],
    by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval, hmodels,
         List.cons_append]⟩
...
theorem execInstruction_sound_push
  ...
  simp only [execInstruction] at hexec
  have heq := Option.some.inj hexec
  have hss : ss' = { stack := .lit v :: ss.stack } := (congrArg Prod.fst heq).symm
  have hpc : preconds = [] := (congrArg Prod.snd heq).symm
  subst hss; subst hpc
  exact ⟨cs.withStack (v :: cs.stack),
    by simp only [MidenLean.execInstruction, execPush, hmodels, MidenState.withStack],
    by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval, hmodels,
         List.cons_append]⟩
...
theorem execInstruction_sound_pushList
  ...
  simp only [execInstruction] at hexec
  have heq := Option.some.inj hexec
  have hss : ss' = { stack := vs.map .lit ++ ss.stack } := (congrArg Prod.fst heq).symm
  have hpc : preconds = [] := (congrArg Prod.snd heq).symm
  subst hss; subst hpc
  exact ⟨cs.withStack (vs ++ cs.stack),
    by simp only [MidenLean.execInstruction, execPushList, hmodels, MidenState.withStack],
    by simp only [State.models, MidenState.withStack, List.map_append, hmodels, List.append_assoc]
       congr 1; exact (map_lit_map_eval σ vs).symm⟩
...
theorem execInstruction_sound_dropw
  ...
  | _ :: _ :: _ :: _ :: tail =>
    ...
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons, List.map_cons, List.map_cons, List.map_cons] at hmodels
    exact ⟨cs.withStack (tail.map (Expr.eval σ) ++ rest),
      by simp only [MidenLean.execInstruction, execDropw, hmodels]; rfl,
...
theorem execInstruction_sound_swapdw
  ...
  | a0::a1::a2::a3::b0::b1::b2::b3::c0::c1::c2::c3::d0::d1::d2::d3::tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := c0::c1::c2::c3::d0::d1::d2::d3::a0::a1::a2::a3::b0::b1::b2::b3::tail } :=
      (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk] at hmodels
    simp only [List.map_cons, List.cons_append] at hmodels
    exact ⟨cs.withStack (c0.eval σ::c1.eval σ::c2.eval σ::c3.eval σ::
      ...
      by simp only [MidenLean.execInstruction, execSwapdw, hmodels, MidenState.withStack]; rfl,
...
theorem execInstruction_sound_reversew
  ...
  | a :: b :: c :: d :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := d :: c :: b :: a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons, List.map_cons, List.map_cons, List.map_cons] at hmodels
    exact ⟨cs.withStack (d.eval σ :: c.eval σ :: b.eval σ :: a.eval σ ::
                          tail.map (Expr.eval σ) ++ rest),
      by simp only [MidenLean.execInstruction, execReversew, hmodels]; rfl,
      by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]⟩
...
theorem execInstruction_sound_addImm
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .add a (.lit v) :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons] at hmodels
    exact ⟨cs.withStack ((a.eval σ + v) :: tail.map (Expr.eval σ) ++ rest),
      by simp only [MidenLean.execInstruction, execAddImm, hmodels]; rfl,
      by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]⟩
...
theorem execInstruction_sound_subImm
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .sub a (.lit v) :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons] at hmodels
    exact ⟨cs.withStack ((a.eval σ - v) :: tail.map (Expr.eval σ) ++ rest),
      by simp only [MidenLean.execInstruction, execSubImm, hmodels]; rfl,
      by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]⟩
...
theorem execInstruction_sound_mulImm
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .mul a (.lit v) :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons] at hmodels
    exact ⟨cs.withStack ((a.eval σ * v) :: tail.map (Expr.eval σ) ++ rest),
      by simp only [MidenLean.execInstruction, execMulImm, hmodels]; rfl,
      by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]⟩
...
theorem execInstruction_sound_neg
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .neg a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons] at hmodels
    exact ⟨cs.withStack (-(a.eval σ) :: tail.map (Expr.eval σ) ++ rest),
      by simp only [MidenLean.execInstruction, execNeg, hmodels]; rfl,
      by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]⟩
...
theorem execInstruction_sound_incr
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .add a (.lit 1) :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons] at hmodels
    exact ⟨cs.withStack ((a.eval σ + 1) :: tail.map (Expr.eval σ) ++ rest),
      by simp only [MidenLean.execInstruction, execIncr, hmodels]; rfl,
      by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]⟩
...
theorem execInstruction_sound_eqImm
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .feltEq a (.lit v) :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons] at hmodels
    exact ⟨cs.withStack ((if a.eval σ == v then (1 : Felt) else 0) ::
                          tail.map (Expr.eval σ) ++ rest),
      by simp only [MidenLean.execInstruction, execEqImm, hmodels]; rfl,
      by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]⟩
...
theorem execInstruction_sound_neq
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .feltNeq a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    ...
    · simp only [MidenLean.execInstruction, execNeq, hmodels]; rfl
    · simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]
      congr 1; congr 1; exact bne_if_eq_if (a.eval σ) (b.eval σ) 1 0
...
theorem execInstruction_sound_neqImm
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .feltNeq a (.lit v) :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons] at hmodels
    refine ⟨cs.withStack ((if a.eval σ != v then (1 : Felt) else 0) ::
                           tail.map (Expr.eval σ) ++ rest), ?_, ?_⟩
    · simp only [MidenLean.execInstruction, execNeqImm, hmodels]; rfl
    · simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]
      congr 1; congr 1; exact bne_if_eq_if (a.eval σ) v 1 0
...
theorem execInstruction_sound_lt
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .feltLt a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons, List.map_cons] at hmodels
    exact ⟨cs.withStack ((if (a.eval σ).val < (b.eval σ).val then (1 : Felt) else 0) ::
                          tail.map (Expr.eval σ) ++ rest),
      by simp only [MidenLean.execInstruction, execLt, hmodels]; rfl,
      by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]⟩
...
theorem execInstruction_sound_lte
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .feltLte a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons, List.map_cons] at hmodels
    exact ⟨cs.withStack ((if (a.eval σ).val ≤ (b.eval σ).val then (1 : Felt) else 0) ::
                          tail.map (Expr.eval σ) ++ rest),
      by simp only [MidenLean.execInstruction, execLte, hmodels]; rfl,
      by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]⟩
...
theorem execInstruction_sound_gt
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .feltGt a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons, List.map_cons] at hmodels
    exact ⟨cs.withStack ((if (a.eval σ).val > (b.eval σ).val then (1 : Felt) else 0) ::
                          tail.map (Expr.eval σ) ++ rest),
      by simp only [MidenLean.execInstruction, execGt, hmodels]; rfl,
      by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]⟩
...
theorem execInstruction_sound_gte
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .feltGte a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons, List.map_cons] at hmodels
    exact ⟨cs.withStack ((if (a.eval σ).val ≥ (b.eval σ).val then (1 : Felt) else 0) ::
                          tail.map (Expr.eval σ) ++ rest),
      by simp only [MidenLean.execInstruction, execGte, hmodels]; rfl,
      by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]⟩
...
theorem execInstruction_sound_isOdd
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .feltIsOdd a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons] at hmodels
    exact ⟨cs.withStack ((if (a.eval σ).val % 2 == 1 then (1 : Felt) else 0) ::
                          tail.map (Expr.eval σ) ++ rest),
      by simp only [MidenLean.execInstruction, execIsOdd, hmodels]; rfl,
      by simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]⟩
...
theorem execInstruction_sound_u32Cast
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .lo32 a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons] at hmodels
    ...
    · change execU32Cast cs = _
      unfold execU32Cast
      rw [hmodels]; simp [MidenState.withStack]
    · simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]
...
theorem execInstruction_sound_u32Split
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .lo32 a :: .hi32 a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    rw [hstk, List.map_cons] at hmodels
...
theorem execInstruction_sound_eqw
  ...
  | b0 :: b1 :: b2 :: b3 :: a0 :: a1 :: a2 :: a3 :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .eqw4 a0 a1 a2 a3 b0 b1 b2 b3
                                 :: b0 :: b1 :: b2 :: b3
                                 :: a0 :: a1 :: a2 :: a3 :: tail } :=
      (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    ...
    rw [hstk, List.map_cons, List.map_cons, List.map_cons, List.map_cons,
        List.map_cons, List.map_cons, List.map_cons, List.map_cons] at hmodels
...
theorem execInstruction_sound_assert
  ...
  | a :: tail =>
    ...
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.eqOne a] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons] at hmodels
    have h1 : (a.eval σ) = 1 :=
      hpreconds (.eqOne a) (by rw [hpc]; simp)
...
theorem execInstruction_sound_assertz
  ...
  | a :: tail =>
    ...
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.eqZero a] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons] at hmodels
    have h0 : (a.eval σ) = 0 :=
      hpreconds (.eqZero a) (by rw [hpc]; simp)
...
theorem execInstruction_sound_assertEq
  ...
  | b :: a :: tail =>
    ...
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.feltEq a b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons] at hmodels
    have hab : (a.eval σ) = (b.eval σ) :=
      hpreconds (.feltEq a b) (by rw [hpc]; simp)
...
theorem execInstruction_sound_assertEqw
  ...
  | b0 :: b1 :: b2 :: b3 :: a0 :: a1 :: a2 :: a3 :: tail =>
    ...
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.feltEq a0 b0, .feltEq a1 b1, .feltEq a2 b2, .feltEq a3 b3] :=
      (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.map_cons, List.map_cons,
        List.map_cons, List.map_cons, List.map_cons, List.map_cons] at hmodels
    have h0 : (a0.eval σ) = (b0.eval σ) :=
      hpreconds (.feltEq a0 b0) (by rw [hpc]; simp)
    have h1 : (a1.eval σ) = (b1.eval σ) :=
      hpreconds (.feltEq a1 b1) (by rw [hpc]; simp)
    have h2 : (a2.eval σ) = (b2.eval σ) :=
      hpreconds (.feltEq a2 b2) (by rw [hpc]; simp)
    have h3 : (a3.eval σ) = (b3.eval σ) :=
      hpreconds (.feltEq a3 b3) (by rw [hpc]; simp)
...
theorem execInstruction_sound_u32Assert
  ...
  | a :: tail =>
    ...
    have heq := Option.some.inj hexec
    have hss : ss' = ss := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons] at hmodels
    ...
    · change execU32Assert cs = _
      ...
      rw [hmodels]; simp [ha]
    · simp only [State.models, hstk, hmodels, List.map_cons]
...
theorem execInstruction_sound_u32AssertW
  ...
  | a :: b :: c :: d :: tail =>
    ...
    have heq := Option.some.inj hexec
    have hss : ss' = ss := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b, .isU32 c, .isU32 d] :=
      (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.map_cons, List.map_cons] at hmodels
    have ha : (a.eval σ).isU32 = true :=
    ...
    have hd : (d.eval σ).isU32 = true :=
      hpreconds (.isU32 d) (by rw [hpc]; simp)
...
theorem execInstruction_sound_or
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .feltOr a b :: tail } := (congrArg Prod.fst heq).symm
...
theorem execInstruction_sound_xor
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .feltXor a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isBool a, .isBool b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons] at hmodels
    have ha : (a.eval σ).isBool = true :=
...
theorem execInstruction_sound_not
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .feltNot a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isBool a] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons] at hmodels
    have ha : (a.eval σ).isBool = true :=
      isBool_guard _ (hpreconds (.isBool a) (by rw [hpc]; simp))
    ...
    · change execNot cs = _
      unfold execNot
      rw [hmodels]; simp [ha, MidenState.withStack]
    · simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]
...
theorem execInstruction_sound_div
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .mul a (.inv b) :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.nonzero b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons] at hmodels
    have hnz : (b.eval σ == (0 : Felt)) = false :=
      hpreconds (.nonzero b) (by rw [hpc]; simp)
    refine ⟨cs.withStack ((a.eval σ * (b.eval σ)⁻¹) :: tail.map (Expr.eval σ) ++ rest), ?_, ?_⟩
    · change execDiv cs = _
      unfold execDiv
      rw [hmodels]; simp [hnz, MidenState.withStack]
    · simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval]
...
theorem execInstruction_sound_divImm
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .mul a (.inv (.lit v)) :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.nonzero (.lit v)] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons] at hmodels
    have hnz : (Expr.eval σ (.lit v) == (0 : Felt)) = false :=
      hpreconds (.nonzero (.lit v)) (by rw [hpc]; simp)
...
theorem execInstruction_sound_inv
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .inv a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.nonzero a] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons] at hmodels
    have hnz : (a.eval σ == (0 : Felt)) = false :=
      hpreconds (.nonzero a) (by rw [hpc]; simp)
...
theorem execInstruction_sound_pow2
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .pow2 a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.valLeq a 63] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons] at hmodels
    have hle : (a.eval σ).val ≤ 63 :=
      hpreconds (.valLeq a 63) (by rw [hpc]; simp)
...
theorem execInstruction_sound_u32OverflowAdd
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32AddHi a b :: .u32AddLo a b :: tail } :=
      (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    ...
    · change execU32OverflowAdd cs = _
      ...
      rw [hmodels]; simp [ha, hb, u32WideAdd, MidenState.withStack]
    · simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval,
          List.cons_append]
...
theorem execInstruction_sound_u32WrappingAdd
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32WAdd a b :: tail } :=
      (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
    ...
    · simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval,
          u32WAdd, u32Max, List.cons_append]
...
theorem execInstruction_sound_u32OverflowAdd3
  ...
  | c :: b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Add3Hi a b c :: .u32Add3Lo a b c :: tail } :=
      (congrArg Prod.fst heq).symm
...
theorem execInstruction_sound_u32WrappingAdd3
  ...
  | c :: b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32WAdd3 a b c :: tail } :=
      (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b, .isU32 c] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.map_cons] at hmodels
    ...
    · change execU32WrappingAdd3 cs = _
      ...
      rw [hmodels]; simp [ha, hb, hc, MidenState.withStack]
    · simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval,
          u32Max, List.cons_append]
...
theorem execInstruction_sound_u32WrappingSub
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32WSub a b :: tail } :=
      (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
...
theorem execInstruction_sound_u32WidenMul
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32MulLo a b :: .u32MulHi a b :: tail } :=
      (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
    ...
    · change execU32WidenMul cs = _
      ...
      rw [hmodels]; simp [ha, hb, u32WideMul, MidenState.withStack]
    · simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval,
          List.cons_append]
...
theorem execInstruction_sound_u32WrappingMul
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32WMul a b :: tail } :=
      (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
...
theorem execInstruction_sound_u32WidenMadd
  ...
  | b :: a :: c :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32MaddLo a b c :: .u32MaddHi a b c :: tail } :=
      (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b, .isU32 c] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.map_cons] at hmodels
    ...
    · change execU32WidenMadd cs = _
      ...
      rw [hmodels]; simp [ha, hb, hc, u32WideMadd, MidenState.withStack]
    · simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval,
          List.cons_append]
...
theorem execInstruction_sound_u32WrappingMadd
  ...
  | b :: a :: c :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32WMadd a b c :: tail } :=
      (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b, .isU32 c] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.map_cons] at hmodels
...
theorem execInstruction_sound_u32DivMod
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32DivRem a b :: .u32DivQuot a b :: tail } :=
      (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b, .nonzero b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
      hpreconds (.isU32 a) (by rw [hpc]; simp)
    ...
    · change execU32DivMod cs = _
      ...
      rw [hmodels]; simp [ha, hb, hbval, MidenState.withStack]
    · simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval,
          List.cons_append]
...
theorem execInstruction_sound_u32Div
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32DivQuot a b :: tail } :=
      (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b, .nonzero b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
...
theorem execInstruction_sound_u32Mod
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32DivRem a b :: tail } :=
      (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b, .nonzero b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
...
theorem execInstruction_sound_u32And
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32And a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_u32Or
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Or a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_u32Xor
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Xor a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_u32Not
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Not a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
      hpreconds (.isU32 a) (by rw [hpc]; simp)
...
theorem execInstruction_sound_u32Popcnt
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Popcnt a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_u32Clz
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Clz a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_u32Ctz
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Ctz a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_u32Clo
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Clo a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_u32Cto
  ...
  | a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Cto a :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_u32Shl
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Shl a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b, .valLeq b 31] :=
      (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
    ...
    have hbleq : (b.eval σ).val ≤ 31 :=
      hpreconds (.valLeq b 31) (by rw [hpc]; simp)
    ...
    · change execU32Shl cs = _
      unfold execU32Shl
      rw [hmodels]; simp [ha, hb, hbguard, MidenState.withStack]
    · simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval,
          List.cons_append]
...
theorem execInstruction_sound_u32Shr
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Shr a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b, .valLeq b 31] :=
      (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
...
theorem execInstruction_sound_u32Rotl
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Rotl a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b, .valLeq b 31] :=
      (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
...
theorem execInstruction_sound_u32Rotr
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Rotr a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b, .valLeq b 31] :=
      (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
...
theorem execInstruction_sound_u32ShlImm
  ...
  · simp only [hn, ite_true, decide_true] at hexec
    ...
    | a :: tail =>
      simp only [hstk] at hexec
      have heq := Option.some.inj hexec
      have hss : ss' = { stack := .u32Shl a (.lit (Felt.ofNat n)) :: tail } :=
        (congrArg Prod.fst heq).symm
      have hpc : preconds = [.isU32 a] := (congrArg Prod.snd heq).symm
      subst hss
      rw [hstk, List.map_cons, List.cons_append] at hmodels
      ...
      · change execU32ShlImm n cs = _
        ...
        rw [hmodels]; simp [ha, hnguard, MidenState.withStack]
      · simp only [State.models, MidenState.withStack, List.map_cons, Expr.eval,
            List.cons_append, hnval]
  · simp [hn] at hexec
...
theorem execInstruction_sound_u32ShrImm
  ...
  · simp only [hn, ite_true, decide_true] at hexec
    ...
    | a :: tail =>
      simp only [hstk] at hexec
      have heq := Option.some.inj hexec
      have hss : ss' = { stack := .u32Shr a (.lit (Felt.ofNat n)) :: tail } :=
        (congrArg Prod.fst heq).symm
      have hpc : preconds = [.isU32 a] := (congrArg Prod.snd heq).symm
      subst hss
      rw [hstk, List.map_cons, List.cons_append] at hmodels
...
theorem execInstruction_sound_u32RotlImm
  ...
  · simp only [hn, ite_true, decide_true] at hexec
    ...
    | a :: tail =>
      simp only [hstk] at hexec
      have heq := Option.some.inj hexec
      have hss : ss' = { stack := .u32Rotl a (.lit (Felt.ofNat n)) :: tail } :=
        (congrArg Prod.fst heq).symm
      have hpc : preconds = [.isU32 a] := (congrArg Prod.snd heq).symm
      subst hss
      rw [hstk, List.map_cons, List.cons_append] at hmodels
...
theorem execInstruction_sound_u32RotrImm
  ...
  · simp only [hn, ite_true, decide_true] at hexec
    ...
    | a :: tail =>
      simp only [hstk] at hexec
      have heq := Option.some.inj hexec
      have hss : ss' = { stack := .u32Rotr a (.lit (Felt.ofNat n)) :: tail } :=
        (congrArg Prod.fst heq).symm
      have hpc : preconds = [.isU32 a] := (congrArg Prod.snd heq).symm
      subst hss
      rw [hstk, List.map_cons, List.cons_append] at hmodels
...
theorem execInstruction_sound_u32Lt
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Lt a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_u32Lte
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Lte a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_u32Gt
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Gt a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_u32Gte
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Gte a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_u32Min
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Min a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_u32Max
  ...
  | b :: a :: tail =>
    simp only [hstk] at hexec
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := .u32Max a b :: tail } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [.isU32 a, .isU32 b] := (congrArg Prod.snd heq).symm
    subst hss
    rw [hstk, List.map_cons, List.map_cons, List.cons_append, List.cons_append] at hmodels
    have ha : (a.eval σ).isU32 = true :=
...
theorem execInstruction_sound_dupw
  ...
  | some a, some b, some c, some d =>
    ...
    have heq := Option.some.inj hexec
    have hss : ss' = { stack := a :: b :: c :: d :: ss.stack } := (congrArg Prod.fst heq).symm
    have hpc : preconds = [] := (congrArg Prod.snd heq).symm
    subst hss; subst hpc
    ...
    · simp only [MidenLean.execInstruction, execDupw, hbase_def, hmodels]
  ...
  | _, _, _, _ =>
    ...
    next a' b' c' d' =>
      ...
      have hvd : ss.stack[base + 3] = d' := getElem_of_getElem?_some _ _ _ hgd
      refine ⟨cs.withStack (a'.eval σ :: b'.eval σ :: c'.eval σ :: d'.eval σ :: cs.stack), ?_, ?_⟩
      · simp only [MidenLean.execInstruction, execDupw, hmodels]
        rw [getElem?_map_append_left (Expr.eval σ) ss.stack rest base hna,
            getElem?_map_append_left (Expr.eval σ) ss.stack rest (base + 1) hnb,
            getElem?_map_append_left (Expr.eval σ) ss.stack rest (base + 2) hnc,
            getElem?_map_append_left (Expr.eval σ) ss.stack rest (base + 3) hnd]
        rw [hva, hvb, hvc, hvd]
      · simp only [State.models, MidenState.withStack, List.map_cons, hmodels]; rfl
    next h =>
...
theorem execInstruction_sound_movupw
  ...
  · have htrue : (decide (2 ≤ n) && decide (n ≤ 3)) = true := by
    ...
    · push_neg at hlen
      ...
      have heq := Option.some.inj hexec
      have hss : ss' = { stack := (ss.stack.drop base).take 4 ++ ss.stack.take base ++ ss.stack.drop (base + 4) } :=
        (congrArg Prod.fst heq).symm
      have hpc : preconds = [] := (congrArg Prod.snd heq).symm
      subst hss; subst hpc
      ...
      · change execMovupw n cs = _
        ...
        simp; omega
      · simp only [State.models, MidenState.withStack,
            List.map_append, List.map_take, List.map_drop, List.append_assoc]
...
theorem execInstruction_sound_movdnw
  ...
  · have htrue : (decide (2 ≤ n) && decide (n ≤ 3)) = true := by
    ...
    · push_neg at hlen
      rw [if_neg (by omega)] at hexec
      have heq := Option.some.inj hexec
      have hss : ss' = { stack := (ss.stack.drop 4).take (n * 4) ++ ss.stack.take 4 ++ (ss.stack.drop 4).drop (n * 4) } :=
        (congrArg Prod.fst heq).symm
      have hpc : preconds = [] := (congrArg Prod.snd heq).symm
      subst hss; subst hpc
