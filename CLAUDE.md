...
- **Symbolic automation** (`MidenLean/Symbolic/`): most straight-line and call-bearing procedures are now proved with `miden_vcg`, which decomposes control flow and closes leaves by verified reflection (`miden_reflect`). Callee calls resolve through `@[miden_exec_summary]`-tagged, fuel-parametric `*_exec` theorems. Prefer this style for new proofs; the manual step-lemma style below remains for procedures the automation does not cover.
